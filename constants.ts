...
const generateSchedule = (group: string): MeetEvent[] => {
  ...
  for (let i = -7; i < 7; i++) {
    const d = getDate(i);
    // If today happens to be Jan 27 2026, we handle it below, otherwise normal gen
    if (d !== '2026-01-27') {
  ...
  // FORCE JAN 27, 2026
  const specialDate = "2026-01-27";
  events.push(...generateJan27Special(specialDate, group));
